...
import { deleteAvatarError, deleteAvatarStart, updateUserError, updateUserStart, updateUserSuccess, userSelector,deleteAvatarSuccess} from "../redux/user/userSlice"
    ...
    const handleFileUpload = (file) => {
      ...
      const handleSubmit = async (e)=>{
        ...
        try {
            ...
            setTimeout(()=>setUpdateDone(true), 2000)
        } catch (error) {
            if (error.response) {
      ...
      const handleDeleteImage = async (e)=>{
        ...
        try {
          ...
          if (error.response) {
            ...
            // that falls out of the range of 2xx
            console.error("Response Data:", error.response.data);
            dispatch(deleteAvatarError(error.response.data.message))
            console.error("Response Status:", error.response.status);
            console.error("Response Headers:", error.response.headers);
          } else if (error.request) {
            // The request was made but no response was received
            console.error("No response received from server:", error.request);
            dispatch(deleteAvatarError(error.request))
          } else {
            // Something happened in setting up the request that triggered an Error
            console.error("Request setup error:", error.message);
            dispatch(deleteAvatarError(error.message))
          }
